...
**「候选」就是脚本写给 agent 的待办便条**：「代码这里变了，某条规则可能跟着过时了，去看一眼，做个决定。」脚本会写生成区块和收件箱，但不替你判断、提升或拒绝事实；agent 逐条处理，四选一：`promoted`（查实了，写进规则）、`checked-unchanged`（看过了，不用改）、`rejected`（不值得成为规则）、`needs-user`（查不了，留给你确认）。
...
- `.agent/rule-candidates.md` 是仓库可见的收件箱和审计记录；严格校验要求它存在。候选处理完后再提交它（最好是没有当前候选的状态），不要把 pending 项留进共享历史。
...
2. **初始扫描（bootstrap）** —— `bootstrap-project-context.py` 扫描当前文件/配置，写下的是**线索，不是结论**：写进 `project-map.md`、`command-contract.md`、`bootstrap-report.md` 和 `rule-candidates.md`，另在 `drift-map.yml`、`.claude/rules/*.md` 和 `adaptation-review.md` 里留下标记清楚的生成区块，等 agent 收紧确认。项目标成 `Status: adapted` 后，bootstrap 不再改 drift/routing 映射，但仍可能刷新生成的证据、命令候选、报告和收件箱内容。一个叫 `sync.ts` 的文件是*信号*，不是「云同步已核实」的证明。
